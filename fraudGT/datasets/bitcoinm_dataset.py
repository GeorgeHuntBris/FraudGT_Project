...
# We still inherit form TemporalDataset as it provides hte scaffolding for the datasets to download(), process(),...
class BitcoinMDataset(TemporalDataset):
    ...
    def process(self):
        ...
        # Load the raw data (has raw.X (node features), raw.edge_index (directed edges, shape [2, 19M]), raw.edge_att (edge_attr, sahpe [19M, 5])
        ...
        # Normalize edge features & get timestamps
        edge_attr = z_norm(raw.edge_attr.float())
        edge_index = raw.edge_index
        timestamps = raw.edge_attr[:, 4].float() # Column 4 before normalisation (timestamp)

        # Stats
        labeled_mask = y >= 0 # Lavels are -1, 0 & 1
        n_labeled = labeled_mask.sum().item()
        n_illicit = (y == 1).sum().item()
        n_licit = (y == 0).sum().item()
        # Print labeled category percentages
        ...
        # Stratified 50/25/25 split on labeled nodes
        # Follows DIAM paper: "ratio 2:1:1"
        labeled_inds = torch.where(labeled_mask)[0].numpy() # Returns the idxs of all Ture values in labeled_mask  & convert to np array. (licit and ilict nodes)
        labeled_y = y[torch.tensor(labeled_inds)].numpy() # Node labels

        # Takes all labelled node indices and splits 50:50. First is for training the second is to be used for val & test split
        train_idx, temp_idx = train_test_split(
            labeled_inds, test_size=0.5, random_state=42, stratify=labeled_y
        )

        # Make val & test split
        temp_y = y[torch.tensor(temp_idx)].numpy() # Get the node labels for the nodes in temp_idx
        val_idx, test_idx = train_test_split(
            temp_idx, test_size=0.5, random_state=42, stratify=temp_y
        )


        # Converts the index arrays (lists of node indices) into boolean mask tensors the same length as the total num of nodes.
        ...
        # Port nums for every edge in the full graph.
        print("\nComputing port numbers (this may take a few minutes for 19M edges)...")
        # # Create empty hetero graph object (only used for ports) and populate
        ...
        # Build adjacency lists from the graph  and use them to assign a port num to every edge in both teh incoming and outgoing directions.
        ...
        for split in ['train', 'val', 'test']:
            ...
            # Create the new graph object and attach all node features and labels.
            data = HeteroData()
            data['node'].x = x
            data['node'].y = y
        ...
        # Check if the dataset was pre-transformed when passed in (consistent with logic in existing codebase)
        # populate data_dict (a python dict with three keys. train, val and test. each value is a hetero graph)
