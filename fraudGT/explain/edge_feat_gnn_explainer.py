...
class EdgeFeatGNNExplainer(ExplainerAlgorithm):
    r"""Extended GNNExplainer that adds an edge feature mask in addition to
    the standard edge structural mask. Designed for graphs where edge features
    carry teh rich information and node features are uninformative (e.g.
    the ETH dataset where node features are placeholder 1s). Based on the GNNExplainer paper:
    ...
    def _create_explanation(self) -> Union[Explanation, HeteroExplanation]:
        ...
        if self.is_hetero:
            ...
            for edge_type, mask in self.edge_feat_mask.items():
                if mask is not None:
                    edge_feat_mask_dict[edge_type] = self._post_process_mask(
                        mask,
                        self.hard_edge_feat_mask[edge_type],
                        apply_sigmoid=True,
                    )
            ...
            explanation.set_value_dict('edge_feat_mask', edge_feat_mask_dict)
    ...
    def _collect_parameters(self, model, edge_index):
        ...
        if self.is_hetero:
            ...
            for mask in self.edge_mask.values():
                if mask is not None:
                    parameters.append(mask)
            # Add edge_feat_mask from all types to parameters
            for mask in self.edge_feat_mask.values():
                if mask is not None:
                    parameters.append(mask)
        else:
            ...
            # Homogenous edge_feat_mask case
    ...
    def _forward_with_masks(
        ...
        **kwargs,
        ...
        if self.is_hetero:
            ...
            # Apply masks to edge attributes
            if edge_attr is not None:
                eh_dict = {}
                for edge_type, attr in edge_attr.items():
                    if edge_type in self.edge_feat_mask and self.edge_feat_mask[edge_type] is not None:
            ...
            # Apply edge feature mask to homogeneous edge attributes
            if edge_attr is not None and self.edge_feat_mask is not None:
                edge_attr = edge_attr * self.edge_feat_mask.sigmoid()
                kwargs = {**kwargs, 'edge_attr': edge_attr}
    ...
    def _initialize_masks(
        ...
        if self.is_hetero:
            ...
            for edge_type, indices in edge_index.items():
                ...
                edge_attr_dim = edge_attr[edge_type].size(1) \
                    if edge_attr is not None and edge_type in edge_attr else 0
                self._initialize_edge_mask(edge_mask_type, edge_type, E, N,
                                           device, edge_attr_dim)
        else:
            # Initialize masks for homogeneous graph
            device = x.device
            (N, F), E = x.size(), edge_index.size(1)
            edge_attr_dim = edge_attr.size(1) if edge_attr is not None else 0

            # Initialize homogeneous node and edge masks
            self._initialize_homogeneous_masks(node_mask_type, edge_mask_type,
                                               N, F, E, device, edge_attr_dim)
    ...
    def _initialize_edge_mask(self, edge_mask_type, edge_type, E, N, device,
                              edge_attr_dim=0):
        """Initialize edge structural mask and edge feature mask."""
        ...
        # Initialize edge feature mask of shape (E, edge_attr_dim)
        # Create a random tensor (random init of edge_feat_mask)
        # Each value drawn form normal dist. (mean =0, std = 1)
        # * 0.1 scales down by 10x, standard practice
        # Wrapped in parameter as should be learned by the optimiser
        if edge_attr_dim > 0:
            self.edge_feat_mask[edge_type] = Parameter(
                torch.randn(E, edge_attr_dim, device=device) * 0.1)
            self.hard_edge_feat_mask[edge_type] = None
        else:
            self.edge_feat_mask[edge_type] = None
            self.hard_edge_feat_mask[edge_type] = None

    def _initialize_homogeneous_masks(self, node_mask_type, edge_mask_type, N,
                                      F, E, device, edge_attr_dim=0):
        """Initialize masks for homogeneous graph."""
        ...
        if edge_mask_type is None:
            ...
            raise ValueError(f"Invalid edge mask type: {edge_mask_type}")
        if edge_attr_dim > 0:
            self.edge_feat_mask = Parameter(
                torch.randn(E, edge_attr_dim, device=device) * 0.1)
            self.hard_edge_feat_mask = None
        else:
    ...
    def _collect_hetero_gradients(self):
        ...
        # Populate hard_edge_feat_mask after first backward pass
        for edge_type, mask in self.edge_feat_mask.items():
            if mask is not None:
                if mask.grad is None:
                    raise ValueError(
                        ...
                        f"used inside the model or disable it via "
                        f"`edge_feat_mask_type=None`.")
                self.hard_edge_feat_mask[edge_type] = mask.grad != 0.0
    ...
    def _collect_homo_gradients(self):
        ...
        # Homogenous edge features case
        if self.edge_feat_mask is not None:
            if self.edge_feat_mask.grad is None:
                raise ValueError("Could not compute gradients for edge "
    ...
    def _apply_hetero_regularization(self, loss):
        ...
        # Apply regularization for each edge feature mask
        for edge_type, mask in self.edge_feat_mask.items():
            if (mask is not None
                    and self.hard_edge_feat_mask[edge_type] is not None):
                loss = self._add_mask_regularization(
                    loss, mask, self.hard_edge_feat_mask[edge_type],
                    self.coeffs['edge_feat_size'],
                    self.coeffs['edge_feat_reduction'],
                    self.coeffs['edge_feat_ent'])
    ...
    def _apply_homo_regularization(self, loss):
        ...
        # Apply regularization for edge feature mask
        if self.hard_edge_feat_mask is not None:
            assert self.edge_feat_mask is not None
            loss = self._add_mask_regularization(
                loss, self.edge_feat_mask, self.hard_edge_feat_mask,
                self.coeffs['edge_feat_size'],
                self.coeffs['edge_feat_reduction'],
                self.coeffs['edge_feat_ent'])
